...
export default async function BoardOgImage({
  ...
  if (thumbnailUrl) {
    // Composite: screenshot background + gradient overlay + text
    return new ImageResponse(
      (
        ...
        >
          ...
          >
            <span style={{ fontSize: 48 }}>{board.icon}</span>
  ...
  // Fallback: gradient + board name (no screenshot available)
  return new ImageResponse(
    (
      <div
        style={{
      ...
      >
        ...
        >
          <span style={{ fontSize: 80 }}>{board.icon}</span>
          <span
            style={{
              fontSize: 48,
              fontWeight: 700,
